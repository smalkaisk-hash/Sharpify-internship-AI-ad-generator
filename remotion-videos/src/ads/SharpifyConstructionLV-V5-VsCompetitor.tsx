...
// V5 HOOK — "Konkurents vs Tu" split screen competitive framing
const SceneHook: React.FC = () => {
  ...
  const leftIn = spring({ frame: frame - 4, fps, config: { damping: 14, stiffness: 130 } });
  const rightIn = spring({ frame: frame - 12, fps, config: { damping: 14, stiffness: 130 } });
  const chip1 = spring({ frame: frame - 24, fps, config: { damping: 16, stiffness: 140 } });
  const chip2 = spring({ frame: frame - 36, fps, config: { damping: 16, stiffness: 140 } });
  const chip3 = spring({ frame: frame - 48, fps, config: { damping: 16, stiffness: 140 } });
  const slam = spring({
    frame: frame - 62,
    fps,
    config: { damping: 10, stiffness: 180, mass: 1.0 },
  });

  const greenChip = (text: string, p: number) => (
    <div
      style={{
        ...
        gap: 12,
        opacity: p,
        transform: `translateY(${interpolate(p, [0, 1], [14, 0])}px) scale(${p})`,
      }}
    >
      <div
        style={{
    ...
    <AbsoluteFill style={{ opacity: outOpacity }}>
      ...
      <div style={{ display: "flex", height: "100%", width: "100%" }}>
        ...
        <div
          style={{
            flex: 1,
            background: "linear-gradient(180deg, #0a2a1a 0%, #060d09 100%)",
        ...
        >
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 10, width: "100%" }}>
            {greenChip("+12 zvani šonedēļ", chip1)}
            {greenChip("+8 pieteikumi", chip2)}
            {greenChip("Kalendārs pilns", chip3)}
          </div>
        </div>
        ...
        <div
          style={{
            width: 8,
            background: `linear-gradient(180deg, ${YELLOW} 0%, ${BLACK} 50%, ${YELLOW} 100%)`,
            boxShadow: `0 0 30px ${YELLOW}60`,
          }}
        />
      ...
      <div
        style={{
          ...
          textAlign: "center",
          opacity: slam,
          transform: `translateY(${interpolate(slam, [0, 1], [20, 0])}px) scale(${slam})`,
        }}
      >
        <div
          style={{
...
export const SharpifyConstructionLVv5: React.FC = () => {
  return (
    <AbsoluteFill style={{ backgroundColor: BLACK }}>
