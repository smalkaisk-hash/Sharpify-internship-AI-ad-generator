...
// V4 HOOK — "Cik tu zaudē?" Calculator of lost revenue
const SceneHook: React.FC = () => {
  ...
  const eyebrow = spring({ frame, fps, config: { damping: 18, stiffness: 100 } });
  const l1 = spring({ frame: frame - 6, fps, config: { damping: 14, stiffness: 130 } });
  const l2 = spring({ frame: frame - 20, fps, config: { damping: 14, stiffness: 130 } });
  const l3 = spring({ frame: frame - 34, fps, config: { damping: 14, stiffness: 130 } });
  ...
  const totalPulse = 1 + Math.sin(frame / 7) * 0.02;

  const line = (op: string, text: string, p: number) => (
    <div
      style={{
        display: "flex",
        alignItems: "baseline",
  ...
  return (
    <AbsoluteFill style={{ opacity: outOpacity }}>
      ...
      <AbsoluteFill
        style={{
          background:
            "radial-gradient(ellipse at 50% 50%, rgba(232,213,0,0.12) 0%, transparent 55%)",
        }}
      />
      ...
      >
        <div
          style={{
            ...
            opacity: eyebrow,
            transform: `translateX(${interpolate(eyebrow, [0, 1], [-20, 0])}px)`,
          }}
        >
          BŪVFIRMAS MATEMĀTIKA
        </div>

        <div style={{ display: "flex", flexDirection: "column", gap: 22 }}>
          {line("", "50 MEKLĒJUMI/DIENĀ", l1)}
          {line("×", "30% KLIENTI ZVANA", l2)}
          {line("×", "€3,000 PROJEKTS", l3)}
          <div
            style={{
              height: 6,
        ...
        <div style={{ textAlign: "center" }}>
          ...
          <div
            style={{
              ...
              opacity: total,
              transform: `scale(${total * totalPulse})`,
              textShadow:
                "0 6px 24px rgba(0,0,0,0.9), 0 0 80px rgba(232,213,0,0.5)",
            }}
          >
            €45,000
          </div>
...
export const SharpifyConstructionLVv4: React.FC = () => {
  return (
    <AbsoluteFill style={{ backgroundColor: BLACK }}>
