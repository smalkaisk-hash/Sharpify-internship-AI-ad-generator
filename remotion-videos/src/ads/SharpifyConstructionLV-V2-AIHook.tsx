...
const SceneHook: React.FC = () => {
  ...
  const zoom = interpolate(frame, [0, 90], [1.0, 1.08], {
    extrapolateRight: "clamp",
  });

  const line1 = spring({ frame: frame - 5, fps, config: { damping: 14, stiffness: 140 } });
  const line2 = spring({ frame: frame - 18, fps, config: { damping: 14, stiffness: 140 } });
  const line3 = spring({ frame: frame - 42, fps, config: { damping: 14, stiffness: 130 } });
  const tiesiTu = spring({
    frame: frame - 55,
    fps,
  ...
  const shake = frame > 55 && frame < 66 ? Math.sin(frame / 1.5) * 3 : 0;

  return (
    <AbsoluteFill style={{ opacity: outOpacity }}>
      {/* Background video — AI-generated bulldozer-through-house (Veo 3 Fast) */}
      <AbsoluteFill style={{ transform: `scale(${zoom})` }}>
        <OffthreadVideo
          src={staticFile("videos/construction-lv/ai-bulldozer-hook.mp4")}
          muted
      ...
      <AbsoluteFill
        style={{
          background:
            "linear-gradient(180deg, rgba(10,10,10,0.75) 0%, rgba(10,10,10,0.4) 30%, rgba(10,10,10,0.55) 60%, rgba(10,10,10,0.9) 100%)",
        }}
      />

      {/* Yellow vignette accent */}
      <AbsoluteFill
        style={{
          background:
            "radial-gradient(ellipse at 50% 55%, rgba(232,213,0,0.12) 0%, transparent 55%)",
        }}
      />
      ...
      >
        ...
        <div
          style={{
            ...
            marginTop: 42,
            opacity: line3,
            transform: `translateX(${interpolate(line3, [0, 1], [-40, 0])}px)`,
            textShadow: "0 4px 20px rgba(0,0,0,0.85)",
          }}
        >
          Iespējams tas esi
        ...
        <div
          style={{
            ...
            letterSpacing: "-0.01em",
            marginTop: 2,
            transform: `scale(${tiesiTu}) translateX(${shake}px)`,
            opacity: tiesiTu,
            textShadow: "0 6px 24px rgba(0,0,0,0.9), 0 0 60px rgba(232,213,0,0.35)",
          }}
        >
          TIEŠI TU.
        </div>
...
export const SharpifyConstructionLVv2: React.FC = () => {
  return (
    <AbsoluteFill style={{ backgroundColor: BLACK }}>
